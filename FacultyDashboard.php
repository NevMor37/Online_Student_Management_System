...
$sql = "select * from tbl_faculty_info t1, tbl_student_evaluation t2, tbl_excel_info t3 where t1.email = t2.FacultyId and t2.Status='Pending' and t1.email = '".$user_email."' and t2.StudentId = t3.PantherId";
...
//			}
			...
			$html .= '<td>'.$Status.'</td>';
			if($Status == 'Complete')
				$html .= '<td><a href="./AppEvaluationForm.php?PantherId='.$PantherId.'&Source=FacultyDB" class="btn btn-primary" disabled>Evaluate</a></td>';
			else
				$html .= '<td><a href="./AppEvaluationForm.php?PantherId='.$PantherId.'&Source=FacultyDB" class="btn btn-primary">Evaluate</a></td>';
			$html .= "</tr>";
		}
// 	}
}
...
$html.="</thead>";
$html.="<tbody>";
$sql = "SELECT * FROM tbl_faculty_info LEFT OUTER JOIN tbl_student_evaluation ON tbl_student_evaluation.FacultyId = tbl_faculty_info.email WHERE tbl_student_evaluation.Status='Complete' and tbl_faculty_info.email='".$user_email."'";
$result = $mysqli->query($sql);
