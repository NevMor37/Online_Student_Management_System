...
<body>
    ...
    <div id="wrapper">
        ...
        <div id="page-wrapper">
            <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header">Update Students Information</h1>
                </div>
            </div>
       <div id="studentInfoDiv" class="row">
       		<!-- #Master Student Information -->
					...
					$html.='Student Records need to Update';
					$html.='</div>';
					...
					$sql = "SELECT distinct * FROM tbl_excel_info  where PantherId not in(select PantherId from tbl_excel_ext_info where PantherId not in (2351637,2328677)) order by PantherId";
...
// 								$html .= '<td>'.$Concentration.'</td>';
								//$html .= '<td>'.$EMail.'</td>';
// 								$html .= '<td>'.$Status.'</td>';
								$html .= '<td><a href="./ModifyEvaluation.php?PantherId='.$PantherId.'" class="btn btn-primary"">Update</a></td>';
								// $html .= '<td>'.$GREDate.'</td>';
					...
					$sql = "SELECT distinct * FROM tbl_excel_info  where PantherId in(select PantherId from tbl_excel_ext_info ) order by PantherId";
					$result = $mysqli->query($sql);
					$row_cnt = $result->num_rows;
						...
						while($row = $result->fetch_assoc()) {
							...
							if($count)
								...
								if($count ==1)
								{
									...
									// 								$html .= '<td>'.$Status.'</td>';
// 									$html .= '<td><a href="./ModifyEvaluation.php?PantherId='.$PantherId.'" class="btn btn-primary"">Update</a></td>';
									$html .= '<td><span class="btn btn-primary disabled"">Updated</span></td>';
									// $html .= '<td>'.$GREDate.'</td>';
									// $html .= '<td>'.$GREVerbalScore.'</td>';
									// $html .= '<td>'.$GREVerbalPercent.'</td>';
									// $html .= '<td>'.$GREQuantScore.'</td>';
